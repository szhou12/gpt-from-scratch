...
max_iters = 5000 # NOTE: v3 - increase training iterations bc lr now is lower
eval_interval = 300
learning_rate = 1e-3 # NOTE: v3 - decrease learning rate bc self-attention can't tolerate high lr
...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        super().__init__()
        # input:n_embd -> output:head_size 这是在利用线性变换, 把embedding vector(“字典”)的长度转化适配为head vector("头")的长度
        self.key = nn.Linear(n_embd, head_size, bias=False)
        self.query = nn.Linear(n_embd, head_size, bias=False)
        self.value = nn.Linear(n_embd, head_size, bias=False)
        # Register "self.tril = torch.tril(torch.ones(block_size, block_size))" into the buffer
        # Why doing so? Because variable "self.tril" is not a model parameter, but is part of the module’s state
    ...
    def forward(self, x):
        ...
        # "Let them talk!" - compute attention scores ("affinities")
        wei = q @ k.transpose(-2, -1) * (C ** (-0.5)) # (B, T, C) @ (B, C, T) -> (B, T, T). (C ** (-0.5)) = scaling factor
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb # (B, T, C=n_embd). B句话, 每句话T个单词, 每个单词用长度=C的向量表示它的token和position信息
        # NOTE: v3 - apply one head of self-attention. (B,T,C=n_embd) -> (B,T,C=n_embd)
        x = self.sa_head(x)
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # NOTE: v3 - crop idx to the last block_size tokens
